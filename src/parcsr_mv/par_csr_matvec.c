...
#if 0
hypre_ParCSRMatrixMatvecMultT( HYPRE_Complex       alpha,
		hypre_ParCSRMatrix *A,
		hypre_ParVector    *x,
		HYPRE_Int k1,
		HYPRE_Complex       beta,
		...
		HYPRE_Int k2)
{
	...
	if (x_tmp == NULL){
		...
		x_tmp = hypre_SeqVectorCreate(2* num_cols_offd );
...
printf("x_tmp actual size %d need %d \n",hypre_VectorSize(x_tmp), num_cols_offd);
	...
	if ((A->x_buf == NULL)||(A->x_buf_size==0)){
		printf("KASIA HYPRE. x-buff is NULL. Allocating x buf size %d, num sends = %d, num recv %d \n", hypre_ParCSRCommPkgSendMapStart
...
printf("x buf size %d need %d \n",A->x_buf_size, hypre_ParCSRCommPkgSendMapStart(comm_pkg,  num_sends) );
	...
	x_buf_data = hypre_CTAlloc(HYPRE_Complex,  hypre_ParCSRCommPkgSendMapStart
			(comm_pkg,  num_sends), HYPRE_MEMORY_SHARED);
#endif
	//printf("x_buf data (send buffer) size is %d x_tmp data (rec buffer) size is %d \n", hypre_ParCSRCommPkgSendMapStart
	//    (comm_pkg,  num_sends), num_cols_offd);

	//printf("KASIA HYPRE  x_buf data (send buffer) size is %d x_tmp data (rec buffer) size is %d \n", hypre_ParCSRCommPkgSendMapStart(comm_pkg,  num_sends), num_cols_offd);
	//fails
	hypre_CSRMatrixMatvecMultOutOfPlace( alpha, diag, x_local,k1, beta, b_local,k3, y_local, k2, 0);
