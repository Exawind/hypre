...
#endif
//static HYPRE_Complex timeMV, timeMVT, timeCopy, timeRelax;
/*--------------------------------------------------------------------------
...
//t3= hypre_MPI_Wtime();
//timeCopy +=(t3-t1);
//hypre_ParCSRMatrixCopyCPUtoGPU(A_array[fine_grid]);
#if 0
...
HYPRE_Complex  nrmFafterGPU =  hypre_ParVectorInnerProdOneOfMult( F_array[fine_grid], 0,  F_array[fine_grid], 0); 
...
HYPRE_Complex  nrmFafter =  hypre_ParVectorInnerProd( F_array[fine_grid], F_array[fine_grid]); 
HYPRE_Complex  nrmVafter =  hypre_ParVectorInnerProd( Vtemp,Vtemp); 
if (my_id == 0) printf("Matrix A size %d %d, Vtemp leght %d Norm of Vtemp before %16.16f (CPU), %16.16f(GPU) and after  %16.16f (STD), %16.16f(GPU) U before %16.16f (CPU) %16.16f (GPU) F before %16.16f (CPU) %16.16f (GPU), U after %16.16f (CPU) %16.16f (GPU) F after %16.16f (CPU) %16.16f (GPU)\n ",
hypre_ParCSRMatrixGlobalNumRows(A_array[fine_grid]),hypre_ParCSRMatrixGlobalNumCols(A_array[fine_grid]), hypre_ParVectorGlobalSize(Vtemp), nrmVbefore, nrmVbeforeGPU, nrmVafter, nrmVafterGPU,
nrmUbefore, nrmUbeforeGPU, nrmFbefore, nrmFbeforeGPU,
nrmUafter, nrmUafterGPU, nrmFafter, nrmFafterGPU);
#endif
//t4 = hypre_MPI_Wtime();
//NOT NEEDED
...
HYPRE_Complex  nrmFafter =  hypre_ParVectorInnerProd( F_array[coarse_grid], F_array[coarse_grid]); 
HYPRE_Complex  nrmFafterGPU =  hypre_ParVectorInnerProdOneOfMult( F_array[coarse_grid],0, F_array[coarse_grid],0); 
#endif
tag++;
/*
 *  HYPRE_Complex       alpha,
...
if (my_id == 0)
	{
		printf("inside Vcycle, all relax CALLs took %16.16f matvecs (prolongation, restriction) took %16.16f, matvecT took %16.16f copying %16.16f \n", timeRelax, timeMV, timeMVT, timeCopy);
	}
#endif
#ifdef HYPRE_USING_CALIPER
