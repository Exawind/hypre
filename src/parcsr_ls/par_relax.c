...
#pragma omp parallel for private(i,ii,j,jj,ns,ne,res,rest,size) HYPRE_SMP_SCHEDULE
						...
						else
						{
							...
							//HERE I PUT MY RELAXATION GPU CODE ---- KS
							//printf("starting \n");
							...
							//need to allocate Vext_data and cf_marker for device
							//printf("copied \n");
							...
							cf_marker_d  = hypre_TAlloc(HYPRE_Int  , n,       HYPRE_MEMORY_DEVICE);
							//printf("allocated\n");
							...
							ParRelax(n, relax_points, cf_marker_d, A_diag->i, A_diag->j, A_diag->data, A_offd->i, A_offd->j, A_offd->data, Vext_data_d, f_local->data, u_local->data);
							//printf("deplpyed\n");

							//hypre_TMemcpy( Vext_data, Vext_data_d, HYPRE_Real, num_cols_offd, HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE );
							hypre_TMemcpy( u_data, u_local->data, HYPRE_Real, n , HYPRE_MEMORY_HOST, HYPRE_MEMORY_DEVICE );
							//printf("copied 3\n");
...
/*
							for (i = 0; i < n; i++)  
							{
										...
										&& A_diag_data[A_diag_i[i]] != zero)
								{
									...
									u_data[i] = (res+u_data[i]) / A_diag_data[A_diag_i[i]];
