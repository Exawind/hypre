...
PUSH_RANGE("GPU L1 Jacobi", 0);
	...
	hypre_ParVectorCopyDataCPUtoGPU(v);
	hypre_ParVectorCopyDataCPUtoGPU(u);
	//works
//	printf("PRECON, just updated GPU parts v %16.16f norm of f %16.16f norm of u %16.16f\n", hypre_ParVectorInnerProdOneOfMult(v, 0, v,0), hypre_ParVectorInnerProdOneOfMult(f, 0, f,0),  hypre_ParVectorInnerProdOneOfMult(u, 0, u,0));
      }

      //copy FROM f to v
      hypre_ParVectorCopy(f, v);
  //    printf("PRECON, just copied from f to v  norm of v %16.16f \n", hypre_ParVectorInnerProdOneOfMult(v, 0, v,0));
      //matvec: v= (u-A*v)*relax_weight = -relax_weight * A + relax_weight*v  
      //works
      //
    //  printf("PRECON, before matvec norm of v %16.16f norm of u %16.16f relax_weight = %16.16f\n", hypre_ParVectorInnerProdOneOfMult(v, 0, v,0), hypre_ParVectorInnerProdOneOfMult(u, 0, u,0), relax_weight);
      hypre_ParCSRMatrixMatvecMult(-1.0*relax_weight, A, u, 0, relax_weight, v, 0);
      //printf("PRECON, after matvec norm of v %16.16f norm of u %16.16f \n", hypre_ParVectorInnerProdOneOfMult(v, 0, v,0), hypre_ParVectorInnerProdOneOfMult(u, 0, u,0));
      //fails 
      //scale: u = (scale by row norms) v


      HYPRE_Int num_rows = hypre_ParCSRMatrixNumRows(A);
      VecScale(hypre_VectorDeviceData(hypre_ParVectorLocalVector(u)),hypre_VectorDeviceData(hypre_ParVectorLocalVector(v)),l1_norms,num_rows,HYPRE_STREAM(4));

      //printf("PRECON, after scale norm of v %16.16f norm of u %16.16f \n", hypre_ParVectorInnerProdOneOfMult(v, 0, v,0), hypre_ParVectorInnerProdOneOfMult(u, 0, u,0));
      //update CPU version of u just in case
      if (sweep == (relax_times-1)){
	hypre_ParVectorCopyDataGPUtoCPU(u);
	hypre_ParVectorCopyDataGPUtoCPU(v);
      }
POP_RANGE;
#else      
	...
	hypre_SeqVectorPrefetchToDevice(hypre_ParVectorLocalVector(f));
      ...
      hypre_ParVectorCopy(f,v);
#endif
      //printf("UNI PRECON, just copied from f to v  norm of v %16.16f \n", hypre_ParVectorInnerProd(v,v));
#ifdef HYPRE_USING_MAPPED_OPENMP_OFFLOAD
      SyncVectorToDevice(hypre_ParVectorLocalVector(v));
#endif
      //printf("UNI PRECON, before matvec norm of v %16.16f norm of u %16.16f relax_weight = %16.16f\n", hypre_ParVectorInnerProd(v, v), hypre_ParVectorInnerProd(u, u), relax_weight);
      hypre_ParCSRMatrixMatvec(-relax_weight, A, u, relax_weight, v);

      //printf("UNI PRECON, after matvec norm of v %16.16f norm of u %16.16f relax_weight = %16.16f\n", hypre_ParVectorInnerProd(v, v), hypre_ParVectorInnerProd(u, u), relax_weight);
      //SyncVectorToHost(hypre_ParVectorLocalVector(v));
...
#pragma omp target teams  distribute  parallel for private(i) num_teams(num_teams) thread_limit(1024)
	...
	u_data[i] += v_data[i] / l1_norms[i];
      ...
      POP_RANGE;
      POP_RANGE;
      //printf("UNI PRECON, after scale norm of v %16.16f norm of u %16.16f relax_weight = %16.16f\n", hypre_ParVectorInnerProd(v, v), hypre_ParVectorInnerProd(u, u), relax_weight);
#endif    
    }//relax type = 1
    ...
    HYPRE_Int *edge_vertex)
{
...
#ifdef HYPRE_NO_GLOBAL_PARTITION
  vert_part = hypre_TAlloc(HYPRE_Int, 2, HYPRE_MEMORY_HOST);
  ...
  HYPRE_Int num_procs;
  hypre_MPI_Comm_size(comm, &num_procs);
  vert_part = hypre_TAlloc(HYPRE_Int, num_procs+1, HYPRE_MEMORY_HOST);
