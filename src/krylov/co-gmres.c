...
{
   hypre_CO-GMRESFunctions * gmres_functions;
   gmres_functions = (hypre_CO-GMRESFunctions *)
      CAlloc( 1, sizeof(hypre_CO-GMRESFunctions), HYPRE_MEMORY_HOST );
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESCreate
 *--------------------------------------------------------------------------*/
 
void *
hypre_CO-GMRESCreate( hypre_CO-GMRESFunctions *gmres_functions )
{
   hypre_CO-GMRESData *gmres_data;
 
   gmres_data = hypre_CTAllocF(hypre_CO-GMRESData, 1, gmres_functions, HYPRE_MEMORY_HOST);
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESDestroy
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESDestroy( void *gmres_vdata )
{
	hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
   HYPRE_Int i;
 
   if (gmres_data)
   {
      hypre_CO-GMRESFunctions *gmres_functions = gmres_data->functions;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESGetResidual
 *--------------------------------------------------------------------------*/

HYPRE_Int hypre_CO-GMRESGetResidual( void *gmres_vdata, void **residual )
{
...
HYPRE_Int
hypre_CO-GMRESSetup( void *gmres_vdata,
                  void *A,
                  void *b,
                  void *x         )
{
...
HYPRE_Int
hypre_CO-GMRESSolve(void  *gmres_vdata,
                 void  *A,
                 void  *b,
		 void  *x)
{
...
/*   FILE           *fp; */
   ...
   if (ieee_check != ieee_check)
   {
      ...
      if (logging > 0 || print_level > 0)
      {
        ...
        hypre_printf("ERROR -- hypre_CO-GMRESSolve: INFs and/or NaNs detected in input.\n");
	...
	if (r_norm  <= epsilon && iter >= min_iter)
        {
           ...
           else /* conv. has not occurred, according to true residual */
           {
              ...
              /* report discrepancy between real/CO-GMRES residuals and restart */
              if ( print_level>0 && my_id == 0)
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetKDim, hypre_CO-GMRESGetKDim
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetKDim( void   *gmres_vdata,
                    HYPRE_Int   k_dim )
{
	hypre_CO-GMRESData *gmres_data =(hypre_CO-GMRESData *) gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetKDim( void   *gmres_vdata,
                    HYPRE_Int * k_dim )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetTol, hypre_CO-GMRESGetTol
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetTol( void   *gmres_vdata,
                   HYPRE_Real  tol       )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetTol( void   *gmres_vdata,
                   HYPRE_Real  * tol      )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetAbsoluteTol, hypre_CO-GMRESGetAbsoluteTol
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetAbsoluteTol( void   *gmres_vdata,
                   HYPRE_Real  a_tol       )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetAbsoluteTol( void   *gmres_vdata,
                   HYPRE_Real  * a_tol      )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetConvergenceFactorTol, hypre_CO-GMRESGetConvergenceFactorTol
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetConvergenceFactorTol( void   *gmres_vdata,
                   HYPRE_Real  cf_tol       )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetConvergenceFactorTol( void   *gmres_vdata,
                   HYPRE_Real * cf_tol       )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetMinIter, hypre_CO-GMRESGetMinIter
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetMinIter( void *gmres_vdata,
                       HYPRE_Int   min_iter  )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetMinIter( void *gmres_vdata,
                       HYPRE_Int * min_iter  )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetMaxIter, hypre_CO-GMRESGetMaxIter
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetMaxIter( void *gmres_vdata,
                       HYPRE_Int   max_iter  )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetMaxIter( void *gmres_vdata,
                       HYPRE_Int * max_iter  )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetRelChange, hypre_CO-GMRESGetRelChange
 *--------------------------------------------------------------------------*/

HYPRE_Int
hypre_CO-GMRESSetRelChange( void *gmres_vdata,
...
HYPRE_Int
hypre_CO-GMRESGetRelChange( void *gmres_vdata,
                         HYPRE_Int * rel_change  )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetSkipRealResidualCheck, hypre_CO-GMRESGetSkipRealResidualCheck
 *--------------------------------------------------------------------------*/

HYPRE_Int
hypre_CO-GMRESSetSkipRealResidualCheck( void *gmres_vdata,
...
HYPRE_Int
hypre_CO-GMRESGetSkipRealResidualCheck( void *gmres_vdata,
                                     HYPRE_Int *skip_real_r_check)
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetStopCrit, hypre_CO-GMRESGetStopCrit
 *
 *  OBSOLETE 
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetStopCrit( void   *gmres_vdata,
                        HYPRE_Int  stop_crit       )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetStopCrit( void   *gmres_vdata,
                        HYPRE_Int * stop_crit       )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetPrecond
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESSetPrecond( void  *gmres_vdata,
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESGetPrecond
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESGetPrecond( void         *gmres_vdata,
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetPrintLevel, hypre_CO-GMRESGetPrintLevel
 *--------------------------------------------------------------------------*/

HYPRE_Int
hypre_CO-GMRESSetPrintLevel( void *gmres_vdata,
                        HYPRE_Int   level)
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetPrintLevel( void *gmres_vdata,
                        HYPRE_Int * level)
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESSetLogging, hypre_CO-GMRESGetLogging
 *--------------------------------------------------------------------------*/

HYPRE_Int
hypre_CO-GMRESSetLogging( void *gmres_vdata,
                      HYPRE_Int   level)
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
HYPRE_Int
hypre_CO-GMRESGetLogging( void *gmres_vdata,
                      HYPRE_Int * level)
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESGetNumIterations
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESGetNumIterations( void *gmres_vdata,
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESGetConverged
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESGetConverged( void *gmres_vdata,
                             HYPRE_Int  *converged )
{
   hypre_CO-GMRESData *gmres_data = (hypre_CO-GMRESData *)gmres_vdata;
...
/*--------------------------------------------------------------------------
 * hypre_CO-GMRESGetFinalRelativeResidualNorm
 *--------------------------------------------------------------------------*/
 
HYPRE_Int
hypre_CO-GMRESGetFinalRelativeResidualNorm( void   *gmres_vdata,
