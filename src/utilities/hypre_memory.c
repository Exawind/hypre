...
static inline HYPRE_Int hypre_RedefMemLocation(HYPRE_Int location)
{
  ...
  if (location == HYPRE_MEMORY_DEVICE)
  {
    ...
    //printf("device memory: original %d and returning %d \n",HYPRE_MEMORY_DEVICE, HYPRE_MEMORY_DEVICE_ACT );
    return HYPRE_MEMORY_DEVICE_ACT;
  }

  if (location == HYPRE_MEMORY_SHARED)
  {
    //printf("original %d and returning %d \n",HYPRE_MEMORY_SHARED, HYPRE_MEMORY_SHARED_ACT );
...
#pragma omp target is_device_ptr(sp)
  ...
  /* cudaMalloc */
  //printf("cuda malloC - allocating %d bytes!\n",  size + sizeof(size_t)*HYPRE_MEM_PAD_LEN );
...
hypre_UnifiedMalloc(size_t size, HYPRE_Int zeroinit)
{
  ...
  cudaError_t err =  cudaMallocManaged(&ptr, count, CUDAMEMATTACHTYPE);;
  //printf("malloc done, error: %d \n", err);
  ...
  //printf("attributes: error: %d \n", err);

  size_t *sp = (size_t*) ptr;
  sp[0] = size;
  ptr = (void*) (&sp[HYPRE_MEM_PAD_LEN]);

  //rr=cudaPointerGetAttributes(&ptr_att,ptr);

  //printf("second check: %d is managed? %d memorytype %d  device %d\n", err, ptr_att.isManaged, ptr_att.memoryType, ptr_att.device );
  ...
  //printf("third check: %d is managed? %d memorytype %d  device %d\n", err, ptr_att.isManaged, ptr_att.memoryType, ptr_att.device );
#endif
...
{
  void * ret = hypre_MAlloc_core(count * elt_size, 1, location);
...
// case 1 - ints
if (elt_size == 4){
//printf("MAKING SURE UNI MEMORY IS 0 for INTS \n");
int * ptr = (int *) ret;
for (int i=0; i<count; ++i){ptr[i]=0;}
}
if (elt_size == 8){
//printf("MAKING SURE UNI MEMORY IS 0.0 for DOUBLES \n");
double * ptr =(double*)  ret;
...
hypre_UnifiedFree(void *ptr)
{
  ...
  /* cudaFree((size_t *) ptr - HYPRE_MEM_PAD_LEN); */
  //printf("this is unified, I am  FREEING IT!\n");  
  cudaSafeFree(ptr, HYPRE_MEM_PAD_LEN);
#endif
}
...
if (location == 0)
  ...
  switch (location)
  {
    ...
    case HYPRE_MEMORY_SHARED :
      /* free unified memory */
//printf("got memory %d  and after converting %d \n", location,hypre_RedefMemLocation(location) );
