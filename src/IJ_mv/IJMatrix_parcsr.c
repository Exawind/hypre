	...
	if (hypre_IJMatrixAssembleFlag(matrix))
	{
		...
		for (ii=0; ii < nrows; ii++)
		{
			...
			if (row >= row_partitioning[pstart] && row < row_partitioning[pstart+1])
			{
				...
				if (n > size)
				{
					//hypre_printf (" row %d too long! n = %d size = %d \n", row, n, size);
					hypre_error(HYPRE_ERROR_GENERIC);
					if (print_level) hypre_printf (" row %d too long! \n", row);
					/* return -1; */
					return hypre_error_flag;
...
hypre_IJMatrixAssembleParCSR(hypre_IJMatrix *matrix)
{
	...
	if (hypre_IJMatrixAssembleFlag(matrix) == 0)
	{
		...
		if (hypre_AuxParCSRMatrixNeedAux(aux_matrix))
		{
			...
			aux_data = hypre_AuxParCSRMatrixAuxData(aux_matrix);
			row_length = hypre_AuxParCSRMatrixRowLength(aux_matrix);
			diag_pos = hypre_CTAlloc(HYPRE_Int,  num_rows, HYPRE_MEMORY_SHARED);
			i_diag = 0;
			i_offd = 0;
#ifdef HYPRE_USING_OPENMP
...
#pragma omp barrier
				...
				if (my_thread_num == 0)
				{
					...
					if (hypre_CSRMatrixJ(diag))
						hypre_TFree(hypre_CSRMatrixJ(diag), HYPRE_MEMORY_SHARED);
					if (hypre_CSRMatrixData(diag))
						hypre_TFree(hypre_CSRMatrixData(diag), HYPRE_MEMORY_SHARED);
					if (hypre_CSRMatrixJ(offd))
						hypre_TFree(hypre_CSRMatrixJ(offd), HYPRE_MEMORY_SHARED);
					if (hypre_CSRMatrixData(offd))
						hypre_TFree(hypre_CSRMatrixData(offd), HYPRE_MEMORY_SHARED);
...
#pragma omp barrier
			...
			hypre_TFree(diag_array, HYPRE_MEMORY_SHARED);
			hypre_TFree(offd_array, HYPRE_MEMORY_SHARED);
			...
			hypre_TFree(diag_pos, HYPRE_MEMORY_SHARED);
		}
		else
		{
...
#pragma omp parallel for private (i,j,j0,temp)
		...
		if (nnz_offd)
		{
			aux_offd_j = hypre_CTAlloc(HYPRE_Int,  nnz_offd, HYPRE_MEMORY_SHARED);
...
//crashes		
		 hypre_TFree(aux_offd_j, HYPRE_MEMORY_SHARED);
		//crashes		 	
		}
